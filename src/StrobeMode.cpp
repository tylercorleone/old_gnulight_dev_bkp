...
StrobeMode::StrobeMode(Gnulight* gnulight, const char* modeName) :
...
bool StrobeMode::interpretUserInteraction(ButtonInteraction& interaction) {
	...
	if (interaction.getClicksCount() > 0) {
		...
		case 2:
			pHostSystem->StopTask(pToggleLightStatusTask);
			pToggleLightStatusTask->setTimeInterval(0);
			currentStrobeType = (currentStrobeType + 1) % STROBE_TYPES_COUNT;
			debug("strobe type " + currentStrobeType);

			if (currentStrobeType == SINUSOIDAL_STROBE || currentStrobeType == LINEAR_STROBE) {
				MainLightLevel currentMainLevel = pHostSystem->advancedLightDriver.getCurrentMainLevel();
				varName = pHostSystem->advancedLightDriver.setMainLevel(currentMainLevel);
				pHostSystem->advancedLightDriver.switchLightStatus(LightStatus::ON);
			}
			pHostSystem->StartTask(pToggleLightStatusTask);
			return true;
		case 3:
			...
			return true;
		case 4:
			if (periodMultiplierX1000 > 125) periodMultiplierX1000 /= 2;
			return true;
		default:
			return false;
		}
...
uint32_t StrobeMode::switchLightStatus(StrobeMode* _this) {
	...
	case ON_OFF_STROBE:
		nextIntervalMs = ON_OFF_STROBE_PERIOD_MS / 2 * _this->periodMultiplierX1000 / 1000;
		break;
	case BEACON_STROBE:
		if (_this->pHostSystem->advancedLightDriver.getLightStatus() == LightStatus::OFF) {
			nextIntervalMs = BEACON_STROBE_PERIOD_MS * BEACON_STROBE_DUTY_CYCLE;
		} else {
			nextIntervalMs = BEACON_STROBE_PERIOD_MS * (1.0f - BEACON_STROBE_DUTY_CYCLE);
		}
		break;
	case DISCO_STROBE:
		if (_this->pHostSystem->advancedLightDriver.getLightStatus() == LightStatus::OFF) {
			...
			nextIntervalMs = DISCO_STROBE_PERIOD_MS * (1.0f - DISCO_STROBE_DUTY_CYCLE);
		}
		break;
	case SINUSOIDAL_STROBE:
		periodMs = PERIODICAL_SEQUENCE_STROBES_PERIOD_MS * _this->periodMultiplierX1000 / 1000;
		nextPotentiometerLevel = MIN_LIGHT_CURRENT_ABOVE_ZERO * 4 +
			(_this->varName - MIN_LIGHT_CURRENT_ABOVE_ZERO * 4) * (sinWave(millis(), periodMs)) / 2.0f;
		break;
	case LINEAR_STROBE:
		periodMs = PERIODICAL_SEQUENCE_STROBES_PERIOD_MS * _this->periodMultiplierX1000 / 1000;
		nextPotentiometerLevel = MIN_LIGHT_CURRENT_ABOVE_ZERO * 4 +
			(_this->varName - MIN_LIGHT_CURRENT_ABOVE_ZERO * 4) * triangularWave(millis(), periodMs);
		break;
	default:
		return -1;
	}
	...
	if (_this->currentStrobeType != SINUSOIDAL_STROBE && _this->currentStrobeType != LINEAR_STROBE) {
		...
		_this->pHostSystem->advancedLightDriver.setPotentiometerLevel(nextPotentiometerLevel);
