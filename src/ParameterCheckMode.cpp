...
ParameterCheckMode::ParameterCheckMode(Gnulight* gnulight, const char* modeName) :
...
bool ParameterCheckMode::onEnterMode(const char* msg) {
	...
	switch (*msg) {
	case BATTERY_CHECK:
		parameterValue = static_cast<int8_t>(pHostSystem->battery.getRemainingCharge() * 10);
		break;
	case LAMP_TEMPERATURE_CHECK:
		parameterValue = pHostSystem->advancedLightDriver.getEmitterTemperature() / 10.0f;
		break;
	default:
		return false;
	}
	...
	pHostSystem->StartTask(pRenderValueWithFlashes);
	return true;
}

void ParameterCheckMode::onExitMode() {
	info(modeName + "::onExitMode");
	pHostSystem->StopTask(pRenderValueWithFlashes);
}
